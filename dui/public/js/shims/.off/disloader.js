// EasyUI Disloader
// Scans DOM for .easyui-* classes and initializes corresponding DaisyUI jQuery plugins
// Grows incrementally as each component is built

(function($) {
  $(document).ready(function() {
    
    // Layout: Two-pane flexbox layout with regions (west/center/east)
    ...
    function duiAjaxGet(url, vars, cb, avar) {
      ...
      var method = (window.dwap && window.dwap.ajax_method) ? window.dwap.ajax_method : 'GET';
    ...
    function fieldValue($field) {
      ...
      // Prefer helper API when available, fallback to direct value.
    ...
    function preloadFkeyCombos($root, source) {
      var $ctx = $root && $root.length ? $root : $(document);
      var $fkeys = $ctx.is('#content') ? $ctx.find('form .fkey') : $ctx.find('#content form .fkey');
      dlog('preloadFkeyCombos', 'found fkey fields', { count: $fkeys.length, source: source || 'unknown', ctx: $ctx.attr('id') || 'document' });
      $fkeys.each(function() {
        var $el = $(this);
        if ($el.hasClass('combobox') || $el.hasClass('easyui-combobox') || $el.is('select')) {
          dlog('preloadFkeyCombos', 'preloading combo options', { id: $el.attr('id') || null, name: $el.attr('name') || null });
          loadComboOptions($el);
        }
      });
    }
    ...
    $(window).off('load.duiFkeyCombo').on('load.duiFkeyCombo', function() {
      preloadFkeyCombos($(document), 'window.load');
    });

    // After panel refresh content injection.
    $(document).off('dui:contentloaded.duiFkeyCombo').on('dui:contentloaded.duiFkeyCombo', function(_, $panel) {
      preloadFkeyCombos($panel, 'dui:contentloaded');
    });
  });
})(jQuery);
