...
//commented by CLS, 2020/6/9 11:15AM
//const cl = console.log

dynjs('/dui/js/xlsx.core.js');
...
$.page.fn.xlbom = function(evt){
  ...
  function getItems(ws,row){
    ...
    */
    ...
    const cols = {
      'NO'    : $.dui.bhave.IMPORT_COL_NO   || 'E',
      'ID'    : $.dui.bhave.IMPORT_COL_ID   || 'G',
      'REV'   : $.dui.bhave.IMPORT_COL_REV  || 'M',
      'DESC'  : $.dui.bhave.IMPORT_COL_DESC || 'N',
      'QTY'   : $.dui.bhave.IMPORT_COL_QTY  || 'AD',
      'UOM'   : $.dui.bhave.IMPORT_COL_UOM  || 'AE'
    };
    ...
    do {
      ...
      var rowdat = {}
      for(var key in cols){
        var col = cols[key].trim();
        var cid = `${col}${row}`;
        var cell = ws[cid];
        if(cell && cell.v){
          if(cell.v == last) {done = true; break;}
          else rowdat[key] = cell.v;
        }
      }      
  ...
  function head(){
    ...
    */
    ...
    var cells = {
      'ID'          : $.dui.bhave.IMPORT_CELL_ID           || 'M3',
      'REV'         : $.dui.bhave.IMPORT_CELL_REV          || 'X3',
      'REV_DATE'    : $.dui.bhave.IMPORT_CELL_REV_DATE     || 'AC3',
      'DESCRIPTION' : $.dui.bhave.IMPORT_CELL_DESCRIPTION  || 'A9'
    }
    
    for(var fid in cells){
      var cid = cells[fid].trim();
      var cell = ws[cid];
      if(cell) data[fid] = cell.v;
    }
