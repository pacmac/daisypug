...
$.page.fn.tonum = function (row) {
  if (row.LINE_INDEX != "") row.LINE_INDEX = parseInt(row.LINE_INDEX);
  if (row.TOTAL_PRICE != "") row.TOTAL_PRICE = parseFloat(row.TOTAL_PRICE);
  if (row.UNIT_PRICE != "") row.UNIT_PRICE = parseFloat(row.UNIT_PRICE);
  if (row.ORDER_QTY != "") row.ORDER_QTY = parseFloat(row.ORDER_QTY);
  return row;
};
...
$.page.fn.itemfunc = {
  totals: function (crow) {
    ...
    var test = /subtotal|runtotal/;
    var rows = $.page.fn._qlines.datagrid("getRows");
    var totcls = $.page.fn.classes()["TOTALS"];
    var cols = totcls ? totcls.QM_COLS.split(",") : [];
    var row,
      data = [],
      subt = 0,
      runt = 0;
    ...
    rows.map(function (row) {
      ...
      // Normal Items
      if (!test.test(row.SOURCE)) {
        if (row.ORDER_QTY && row.UNIT_PRICE) {
          var tot = Math.round(row.UNIT_PRICE * row.ORDER_QTY * 100) / 100;
          if (tot != row.TOTAL_PRICE) {
            row.TOTAL_PRICE = tot;
            row._changed = true;
          }
          subt += row.TOTAL_PRICE;
          runt += row.TOTAL_PRICE;
          runt = Math.round((runt + Number.EPSILON) * 100) / 100;
        }
      }
        ...
        ajaxget(
          ...
          function (res) {
            if (res._NEXT != undefined) {
              $.page.fn._quoteid.qbe("loadForm", res._NEXT);
            }
          }
        );
      }
...
$.page.fn.convert_quote_button = function () {
  ...
  $("#convert_quote").linkbutton(endis);
};
...
// NOTE: $.dui.pdata must be an array (populated by framework from admin^_globs).
// Guard against empty/missing pdata to prevent runtime errors.
...
$.page.fn.formLoad = function (data) {
  ...
  // Set item values
  for (var fn in data) {
    const tbox = form.find(`input[textboxname="${fn}"]`);
...
$.page.fn.formfields = function (row) {
  ...
  for (var fn in fields) {
    const inp = form.find(`input[textboxname="${fn}"]`);
    if (fn == "ID") continue;
...
$.page.fn.onFormOpen = function () {
  ...
  var desc = $('input[textboxname="DESCRIPTION"]');
...
// Columns with JS-driven formatters/editors (merged over JSON config)
$.page.fn.columns = [
...
$(document).ready(function () {
  ...
  $("form#qhead")
    ...
    .on("loadDone", function (jq, data) {
      ...
      $("#save_as_template").linkbutton("enable");
  ...
  $.page.fn._qlines
    .datagrid("options")
    .dlg.window({ onOpen: $.page.fn.onFormOpen });
