$.page.ready(function () {
  ...
  $.page.fn.bcemp = function (val) {
    ...
    $.dui.ajax.ajaxget('/', { _func: 'get', _sqlid: 'vwltsa^empall', ID: empId, SITE_ID: 'DEFAULT' }, function (emp) {
      if (!emp || emp === {}) return;
  ...
  $('#PART_ID').combobox({
    ...
    loadFilter: function (data) {
      ...
      return (data || []).filter(function (d) {
        ...
        return d.BAL_QTY > 0 && d.TRACEABLE === 'N' && d.DIM_TRACKED === 'N' && val.indexOf(partFilter) === 0;
      });
    },
    onSelect: function (v) {
      var nn = $('#QTY');
  ...
  $('#OPERATOR_ID').combobox({
    ...
    onSelect: function (v) {
      $.dui.ajax.ajaxget('/', { _func: 'get', _sqlid: 'vwltsa^empall', ID: v.value, SITE_ID: 'DEFAULT' }, function (emp) {
        if (!emp || emp === {}) return;
        $('#OPERATOR_NAME').val(emp.FIRST_NAME + ' ' + emp.LAST_NAME);
        $.page.fn.loadPending(emp.ID);
      });
  ...
  if (typeof $.dui.fn.bcscan === 'function') {
    $.dui.fn.bcscan(function (bc) {
      switch (bc.pre) {
        case '1':
          var emp = $('#OPERATOR_ID');
          if (emp.combobox('exists', bc.data)) {
