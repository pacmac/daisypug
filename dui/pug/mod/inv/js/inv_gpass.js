$.page.ready(function () {
  ...
  $.page.register({
    hooks: {
      afterLoad: function (data) {
        console.log("page.register.hooks,afterLoad", data);
        $("#gplines").datagrid("load", {
          _func: "get",
          _sqlid: "inv^gplines",
          _dgrid: "y",
          GP_ID: data.GP_ID,
        ...
        var doc = [
          { value: doctype, text: doctype },
          { value: "NIL", text: "NIL" },
        ];
        //prevent user click on DocID if DocType not selected

        var dgDocType = $("#gplines_editor [name=DOC_TYPE");
        var qbeDocId = $("#gplines_editor [name=DOC_ID");
          ...
          onSelect: function (rec) {
            if (rec.value == "NIL") {
              ...
              if (rec.value == "SO") {
                ...
                qbeDocId.qbe("options").queryParams.CUST_ID =
                  $("#GP_TYPE_ID").combobox("getValue");
              } else {
                qbeDocId.qbe("options").queryParams._sqlid =
                  "inv^gp_opnrefs_qbe";
                if (qbeDocId.qbe("options").queryParams.CUST_ID)
                  delete qbeDocId.qbe("options").queryParams.CUST_ID;
              }
    ...
    columns: [
      [
          ...
          editor: {
            ...
            options: {
              ...
              fields: [
                ...
                {
                  field: "USER_1",
                  title: "Cust Prod Order #",
                  width: 150,
                  fixed: true,
                },
              ],
              onSelect: function (rec) {
                $("#gplines_editor [name=PART_ID").textbox(
                  "setValue",
                  rec.PART_ID,
                );
                $("#gplines_editor [name=PART_DESCRIPTION").textbox(
                  "setValue",
                ...
                switch (dgDocType.combobox("getValue")) {
                  case "SO":
                    ...
                    $("#gplines_editor [name=DOC_ID").textbox(
                      "setValue",
                      docId,
                    );
                    $("#gplines_editor [name=QTY").numberspinner("set", {
                      min: 0,
                      max: docQty,
                    });
                    $("#gplines_editor [name=BAL_QTY").numberbox(
                      "setValue",
                      docQty,
                    );
                    ...
                    $("#gplines_editor [name=CUST_PROD_ORDER_NO").textbox(
                      "setValue",
                      rec.USER_1,
                    );
                    $("#gplines_editor [name=CUST_PART_ID").textbox(
                      "setValue",
                      rec.CUST_PART_ID,
                    );
                  ...
                  case "WO":
                    ...
                    ajaxget(
                      "/",
                      {
                        _func: "get",
                        _sqlid: "vwltsa^sub",
                        WOREF: JobId[0] + "^" + JobId[1],
                      },
                      function (rs) {
                        var txType = $("#TX_TYPE").combobox("getValue");
                        if (txType == "S")
                          var docQty = rs.DESIRED_QTY - rs.GATEPASS_QTY;
          ...
          editor: {
            ...
            options: {
              ...
              onSelect: function (rw) {
                ...
                $("#gplines_editor [name=PART_UOM").textbox(
                  "setValue",
                  rw.UOM_ID,
                );
              },
        ...
        },
        {
          ...
          editor: {
            type: "numberbox",
            options: { precision: 2, readonly: true },
          },
        },
        {
          field: "PART_DESCRIPTION",
          ...
          editor: {
            type: "textbox",
            options: { readonly: false, multiline: true, height: 150 },
          },
          coloff: true,
        },
      ],
  ...
  $("#GP_TYPE").combobox({
    ...
    onSelect: function (rec) {
      ...
      var cv = $("#GP_TYPE_ID");
      cv.combobox("clear");
      cv.combobox("options").queryParams._sqlid = {
        C: "sales^custid",
        V: "inv^vendorid",
      }[rec.value];
      cv.combobox("reload");
  ...
  $("#GP_TYPE_ID").combobox({
    ...
    onSelect: function (rec) {
      var cvall = $(this)
        .combobox("options")
        .queryParams._sqlid.replace(/id$/, "all");
      ajaxget(
        "/",
        {
          _sqlid: cvall,
        ...
        function (fields) {
          ...
          for (var id in fields)
            $("[name=" + id + "]").textbox("setValue", fields[id]);
        },
      );
    },
  });
  ...
  $("form#gphead")
    ...
    .on("done", function (data) {
      if (!data) data = { GP_ID: $("#GP_ID").textbox("getValue") };
      $.page.hook("afterLoad", data);
    });
}); // $.page.ready
