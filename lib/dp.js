...
(function(global) {
  ...
  class DpPopup extends DpContainer {
    open(anchor) {
      ...
      if (anchor) {
        ...
        if (anchorEl instanceof DpComponent) anchor = anchorEl.el;
        if (anchorEl) {
          ...
          if (this.el.classList.contains('dp-popup-end')) {
  ...
  class DpDataTable extends DpTable {
    ...
    _updatePaginationUI() {
      ...
      // Build compact pagination bar
      const join = document.createElement('div');
      join.className = 'dp-join join';

      // Prev button
      const prevBtn = document.createElement('button');
      prevBtn.className = 'dp-btn btn btn-sm join-item' + (current <= 1 ? ' btn-disabled' : '');
      prevBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m15 18-6-6 6-6"/></svg>';
      prevBtn.addEventListener('click', () => { if (current > 1) this.setPage(current - 1); });
      join.appendChild(prevBtn);

      // Page info
      const info = document.createElement('span');
      info.className = 'dp-btn btn btn-sm join-item btn-ghost pointer-events-none';
      info.textContent = `${current} / ${pageCount}`;
      join.appendChild(info);

      // Next button
      const nextBtn = document.createElement('button');
      nextBtn.className = 'dp-btn btn btn-sm join-item' + (current >= pageCount ? ' btn-disabled' : '');
      nextBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 18 6-6-6-6"/></svg>';
      nextBtn.addEventListener('click', () => { if (current < pageCount) this.setPage(current + 1); });
      join.appendChild(nextBtn);

      // Row count
      if (total > 0) {
        const count = document.createElement('span');
        count.className = 'dp-btn btn btn-sm join-item btn-ghost pointer-events-none opacity-60 text-xs';
        count.textContent = `${total} rows`;
        join.appendChild(count);
      }
