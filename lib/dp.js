...
(function(global) {
  ...
  // ============================================================
  // DpCombobox — custom dropdown with data store, remote loading
  // ============================================================

  const _CB_CHEVRON = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4"><path fill-rule="evenodd" d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd"/></svg>';

  class DpCombobox extends DpComponent {
    _cbData = null;    // loaded data rows [{value, text, ...}]
    _cbOpts = { valueField: 'value', textField: 'text' };
    _open = false;
    _display = null;   // text input (shows selected text)
    _hidden = null;    // hidden input (carries form value + name)
    _toggle = null;    // chevron button
    _dropdown = null;  // dropdown panel (ul)
    _filtered = null;  // filtered subset for rendering (null = show all)
    _debounceTimer = null;
    _renderOffset = 0; // lazy render: how many items already rendered
    _sentinel = null;  // IntersectionObserver sentinel element
    _observer = null;  // IntersectionObserver instance
    _hlIndex = -1;     // keyboard highlight index

    // --- Shell: find or build DOM parts ---

    _ensureStructure() {
      if (this.el.tagName === 'SELECT') {
        this._upgradeFromSelect();
      } else {
        this._findOrBuildParts();
      }
      // Ensure wrapper has position for absolute dropdown
      if (!this.el.style.position) this.el.style.position = 'relative';
    }

    _findOrBuildParts() {
      const el = this.el;
      this._display = el.querySelector('.dp-cb-display') || el.querySelector('input:not([type="hidden"])');
      this._hidden = el.querySelector('input[type="hidden"]');
      this._toggle = el.querySelector('.dp-cb-toggle');
      this._dropdown = el.querySelector('.dp-cb-dropdown');
      this._selected = el.querySelector('.dp-cb-selected');

      if (!this._display) {
        this._display = document.createElement('input');
        this._display.type = 'text';
        this._display.readOnly = true;
        this._display.classList.add('dp-cb-display', 'input', 'w-full');
        el.prepend(this._display);
      }

      if (!this._selected) {
        const sel = document.createElement('div');
        sel.classList.add('dp-cb-selected');
        this._display.after(sel);
        this._selected = sel;
      }
      ...
      if (!this._hidden) {
        ...
        if (this._display.name) {
          this._hidden.name = this._display.name;
          this._display.removeAttribute('name');
        }
        this._selected.after(this._hidden);
      }

      if (!this._toggle) {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.classList.add('dp-cb-toggle');
        btn.tabIndex = -1;
        btn.innerHTML = _CB_CHEVRON;
        this._hidden.after(btn);
        this._toggle = btn;
      }

      if (!this._dropdown) {
        const dd = document.createElement('ul');
        dd.classList.add('dp-cb-dropdown', 'menu');
        el.appendChild(dd);
        this._dropdown = dd;
      }
    }
    ...
    _upgradeFromSelect() {
      ...
      // Transfer classes — keep dp-combobox, drop select-specific ones
      for (const cls of select.classList) {
        if (cls === 'dp-select' || cls.startsWith('select-') || cls === 'select') continue;
        wrapper.classList.add(cls);
      }
      wrapper.classList.add('dp-combobox');
      if (select.id) { wrapper.id = select.id; select.removeAttribute('id'); }

      // Hidden input — carries name + form value
      this._hidden = document.createElement('input');
      this._hidden.type = 'hidden';
      this._hidden.name = select.name || '';
      this._hidden.value = select.value || '';
      if (select.required) this._hidden.required = true;

      // Display input — shows selected text
      this._display = document.createElement('input');
      this._display.type = 'text';
      this._display.readOnly = true;
      this._display.classList.add('dp-cb-display', 'input', 'input-bordered', 'input-sm', 'w-full');
      const selOpt = select.options?.[select.selectedIndex];
      if (selOpt && !selOpt.disabled) this._display.value = selOpt.text;
      const ph = select.querySelector('option[disabled][selected]');
      if (ph) this._display.placeholder = ph.text;

      // Selected overlay — renders rich content for selected item
      this._selected = document.createElement('div');
      this._selected.classList.add('dp-cb-selected');

      // Toggle button
      this._toggle = document.createElement('button');
      this._toggle.type = 'button';
      this._toggle.classList.add('dp-cb-toggle');
      this._toggle.tabIndex = -1;
      this._toggle.innerHTML = _CB_CHEVRON;

      // Dropdown panel
      this._dropdown = document.createElement('ul');
      this._dropdown.classList.add('dp-cb-dropdown', 'menu');

      // Assemble
      wrapper.append(this._display, this._selected, this._hidden, this._toggle, this._dropdown);

      // Carry over pending data/config from editor modal
      if (select._pendingData) wrapper._pendingData = select._pendingData;
      if (select._pendingConfig) wrapper._pendingConfig = select._pendingConfig;

      // Swap in DOM
      select.parentNode.replaceChild(wrapper, select);
      this.el = wrapper;
      _cache.delete(select);
    }
    ...
    open() {
      if (this._open || this._display?.disabled) return this;
      this._open = true;
      this.el.classList.add('dp-cb-open');
      this._dropdown.style.display = '';
      this._hlIndex = -1;
      // Hide selected overlay so input is visible for search
      if (this._selected) {
        this._selected.style.display = 'none';
        this._display.style.color = '';
      }
      // Scroll active item into view
      const active = this._dropdown.querySelector('li a.active');
    ...
    close() {
      ...
      this._open = false;
      this.el.classList.remove('dp-cb-open');
      this._dropdown.style.display = 'none';
      // Restore display text to selected value (clears search query)
      if (!this._display.readOnly && this._cbData) {
        const vf = this._cbOpts.valueField || 'value';
        const val = this.getValue();
        const row = val ? this._cbData.find(r => String(r[vf]) === String(val)) : null;
        this._updateDisplay(row);
      }
      // Reset filter
      if (this._filtered) {
        this._filtered = null;
    ...
    toggleDropdown() { return this._open ? this.close() : this.open(); }
    ...
    setValue(val) {
      ...
      // Update display from data if loaded
      if (this._cbData) {
        const vf = this._cbOpts.valueField || 'value';
        const row = this._cbData.find(r => String(r[vf]) === String(val));
        this._updateDisplay(row);
    ...
    getText() { return this._display?.value ?? ''; }
    setText(text) { if (this._display) this._display.value = text; return this; }

    getName() { return this._hidden?.name ?? ''; }
    setName(str) { if (this._hidden) this._hidden.name = str; return this; }

    clear() {
      if (this._hidden) this._hidden.value = '';
      if (this._display) { this._display.value = ''; this._display.style.color = ''; }
      if (this._selected) { this._selected.textContent = ''; this._selected.style.display = 'none'; }
      return this;
    }

    // --- Focus / Disable ---

    focus() { this._display?.focus(); return this; }
    blur() { this._display?.blur(); return this; }

    enable() {
      if (this._display) this._display.disabled = false;
      this.el.classList.remove('btn-disabled');
      return this;
    }

    disable() {
      if (this._display) this._display.disabled = true;
      this.close();
      this.el.classList.add('btn-disabled');
      return this;
    }

    isEnabled() { return this._display ? !this._display.disabled : true; }
    isReadOnly() { return this._display?.readOnly ?? false; }
    setReadOnly(bool) { if (this._display) this._display.readOnly = bool; return this; }
    getPlaceholder() { return this._display?.placeholder ?? ''; }
    setPlaceholder(str) { if (this._display) this._display.placeholder = str; return this; }
    isRequired() { return this._hidden?.required ?? false; }
    setRequired(bool) { if (this._hidden) this._hidden.required = bool; return this; }

    // --- Config ---

    config(opts) {
      Object.assign(this._cbOpts, opts);
      // editable: true makes the display input typeable (searchable)
      if (opts.editable !== undefined && this._display) {
        this._display.readOnly = !opts.editable;
      }
      return this;
    }
    ...
    loadData(data) {
      let rows = Array.isArray(data) ? data : (data?.rows || data?.data || data?.items || []);
      if (this._cbOpts.loadFilter) rows = this._cbOpts.loadFilter(rows);

      const vf = this._cbOpts.valueField || 'value';
      const tf = this._cbOpts.textField || 'text';

      this._cbData = rows.map(r => {
        const row = { ...r };
        if (row[vf] === undefined) row[vf] = r.value ?? r.VALUE ?? r.id ?? r.ID ?? '';
        if (row[tf] === undefined) row[tf] = r.text ?? r.TEXT ?? r.name ?? r.NAME ?? row[vf] ?? '';
        return row;
      });

      this._renderItems();

      // Auto-select queued value
      if (this._autoload !== undefined) {
        this.setValue(this._autoload);
        this._autoload = undefined;
      }

      this.emit('dp:loadsuccess', this._cbData);
      return this;
    }
    ...
    _renderItems() {
      ...
      const data = this._filtered || this._cbData;
      if (!data?.length) return;

      const batchSize = this._cbOpts.pageSize || 50;
      this._renderBatch(data, 0, batchSize);

      // Set up lazy loading if more items remain
      if (data.length > batchSize) {
        this._setupObserver(data, batchSize);
      }
    }

    _renderBatch(data, start, count) {
      const dd = this._dropdown;
      const vf = this._cbOpts.valueField || 'value';
      const tf = this._cbOpts.textField || 'text';
      const fmt = this._cbOpts.formatter;
      const curVal = this.getValue();
      const end = Math.min(start + count, data.length);

      // Template-clone li > a items
      ...
      for (let i = start; i < end; i++) {
        ...
        if (fmt) {
          const result = fmt(row, anchor);
          if (typeof result === 'string') anchor.textContent = result;
        } else {
          anchor.textContent = row[tf];
        }
        item.dataset.value = String(row[vf]);
        if (String(row[vf]) === curVal) anchor.classList.add('active');
        frag.appendChild(item);
      }

      // Insert before sentinel if it exists, otherwise append
      if (this._sentinel?.parentNode === dd) {
        dd.insertBefore(frag, this._sentinel);
      } else {
        dd.appendChild(frag);
      }
      this._renderOffset = end;
    }

    _setupObserver(data, batchSize) {
      // Sentinel element at the bottom of the dropdown
      this._sentinel = document.createElement('li');
      this._sentinel.classList.add('dp-cb-sentinel');
      this._sentinel.style.height = '1px';
      this._dropdown.appendChild(this._sentinel);

      this._observer = new IntersectionObserver((entries) => {
        if (!entries[0].isIntersecting) return;
        if (this._renderOffset >= data.length) {
          this._teardownObserver();
          return;
        }
        this._renderBatch(data, this._renderOffset, batchSize);
        if (this._renderOffset >= data.length) {
          this._teardownObserver();
        }
      }, { root: this._dropdown, threshold: 0 });
    ...
    async load(url, opts = {}) {
      if (opts.valueField || opts.textField || opts.loadFilter || opts.queryParams) {
        this.config(opts);
      }
      this._cbOpts.url = url || this._cbOpts.url;

      try {
        const method = (opts.method || 'GET').toUpperCase();
        const qp = this._cbOpts.queryParams;
        let fetchUrl = this._cbOpts.url;
        if (qp && method === 'GET') {
          const sep = fetchUrl.includes('?') ? '&' : '?';
          fetchUrl += sep + new URLSearchParams(qp).toString();
        }
        const result = await _fetch(method, fetchUrl, method !== 'GET' ? qp : null);
        this.loadData(result.data);
      } catch (err) {
        this.emit('dp:loaderror', err);
      }
      return this;
    }

    async reload(url) {
      return this.load(url || this._cbOpts.url);
    }

    getData() { return this._cbData ? this._cbData.slice() : []; }

    getRec(value) {
      if (!this._cbData) return null;
      const v = value !== undefined ? value : this.getValue();
      if (v === '' || v === null || v === undefined) return null;
      const vf = this._cbOpts.valueField || 'value';
      return this._cbData.find(r => String(r[vf]) === String(v)) || null;
    }
    ...
    onSelect(fn) { return this.on('dp:select', fn); }
    onLoadSuccess(fn) { return this.on('dp:loadsuccess', fn); }
    onLoadError(fn) { return this.on('dp:loaderror', fn); }
    ...
    _initComboEvents() {
      ...
      // Display input click — open (or toggle if readonly)
      this._display.addEventListener('click', (e) => {
        e.stopPropagation();
        if (this._display.readOnly) {
          this.toggleDropdown();
        } else if (!this._open) {
          this.open();
        }
      ...
      // Type-to-search on input event
      this._display.addEventListener('input', () => {
        const q = this._display.value.trim();
        if (!this._open) this.open();
        this._applyFilter(q);
      });

      // On focus, select all text for easy replacement (searchable mode)
      this._display.addEventListener('focus', () => {
        if (!this._display.readOnly) this._display.select();
      });
      ...
      this._display.addEventListener('keydown', (e) => {
        ...
        if (key === 'Enter') {
          e.preventDefault();
          if (this._open && this._hlIndex >= 0) {
            const items = this._dropdown.querySelectorAll('li[data-value]');
            if (items[this._hlIndex]) {
              this._selectItem(items[this._hlIndex]);
              this.close();
            }
          } else if (!this._open) {
            this.open();
        ...
        if (key === 'ArrowDown' || key === 'ArrowUp') {
          e.preventDefault();
          if (!this._open) { this.open(); this._hlIndex = -1; }
          const items = this._dropdown.querySelectorAll('li[data-value]');
          if (!items.length) return;
          if (key === 'ArrowDown') {
            this._hlIndex = this._hlIndex < items.length - 1 ? this._hlIndex + 1 : 0;
          } else {
            this._hlIndex = this._hlIndex > 0 ? this._hlIndex - 1 : items.length - 1;
          }
          this._updateHighlight(items);
          return;
        }
        if (key === 'Tab' && this._open) {
          this.close();
        }
      });
    ...
    _updateHighlight(items) {
      items.forEach((li, i) => {
        ...
        if (a) a.classList.toggle('dp-cb-highlight', i === this._hlIndex);
      });
      // Scroll highlighted item into view
      if (items[this._hlIndex]) {
        items[this._hlIndex].scrollIntoView({ block: 'nearest' });
      }
    }
    ...
    _applyFilter(query) {
      ...
      // Remote mode — debounced server query
      if (this._cbOpts.url && this._cbOpts.mode === 'remote') {
        this._debounceTimer = setTimeout(() => {
          const qp = { ...(this._cbOpts.queryParams || {}), q: query };
          this._cbOpts.queryParams = qp;
          this.load(this._cbOpts.url).then(() => {
            this._cbOpts.queryParams = qp; // preserve for next query
          });
        }, this._cbOpts.delay || 300);
        return;
      }

      // Local mode — case-insensitive substring filter
      const tf = this._cbOpts.textField || 'text';
      const vf = this._cbOpts.valueField || 'value';
      const lq = query.toLowerCase();
      const filterFn = this._cbOpts.filter || ((row) => {
        const text = String(row[tf] || '').toLowerCase();
        const val = String(row[vf] || '').toLowerCase();
        return text.includes(lq) || val.includes(lq);
      });

      this._filtered = this._cbData ? this._cbData.filter(filterFn) : [];
      this._renderItems();
    }

    _updateDisplay(rec) {
      const tf = this._cbOpts.textField || 'text';
      const plainText = rec ? (rec[tf] ?? '') : '';

      if (!rec || !this._cbOpts.displayFormatter) {
        // No formatter — plain text in input, hide overlay
        this._display.value = plainText;
      ...
      // displayFormatter renders into the overlay element
      this._selected.textContent = '';
      this._cbOpts.displayFormatter(rec, this._selected);
      this._selected.style.display = '';
      // Make input text transparent (still holds value for search/form)
      this._display.value = plainText;
      this._display.style.color = 'transparent';
      return plainText;
    }
    ...
    _selectItem(li) {
      ...
      const vf = this._cbOpts.valueField || 'value';
      const rec = this._cbData?.find(r => String(r[vf]) === val) || null;
      const tf = this._cbOpts.textField || 'text';

      this._hidden.value = val;
      const text = this._updateDisplay(rec) || li.firstElementChild?.textContent || '';

      // Update active state
      this._dropdown.querySelectorAll('li a.active').forEach(a => a.classList.remove('active'));
      const anchor = li.firstElementChild;
      if (anchor) anchor.classList.add('active');

      this.emit('dp:select', { value: val, text, rec });
    }
  }
  ...
  class DpDropdown extends DpToggleable {
    ...
    _items() { return Array.from(this.el.querySelectorAll('.dp-dropdown-item, .dp-dropdown-content > li')); }
    ...
    getItemTexts() {
      return this._items().map(el => el.textContent.trim());
    }

    addItem(opts) {
      const ul = this.el.querySelector('.dp-dropdown-content, .dropdown-content');
      if (!ul) return this;
      const li = document.createElement('li');
      li.className = 'dp-dropdown-item';
      const a = document.createElement('a');
      if (opts.href) a.href = opts.href;
      if (opts.page) a.dataset.page = opts.page;
      if (opts.icon) {
        ...
        if (window.lucide) setTimeout(() => lucide.createIcons({ nodes: [icon] }), 0);
      }
      if (opts.text) a.appendChild(document.createTextNode(opts.text));
      li.appendChild(a);
      ul.appendChild(li);
      return this;
    }
    ...
    onItemClick(fn) {
      ...
      if (ul) ul.addEventListener('click', (e) => {
        ...
        if (!li) return;
        const idx = this._items().indexOf(li);
        const a = li.querySelector('a');
        const page = a?.dataset.page || null;
        const text = li.textContent.trim();
        this.close();
        fn({ text, page, index: idx, href: a?.href }, e, this);
      });
      return this;
    }
  }
  ...
  class DpTable extends DpComponent {
    ...
    _rows() { return Array.from((this._tbody() || this.el).querySelectorAll('tr:not(.dp-group-header)')); }

    // --- Column config (datagrid mode) ---
    _columns = null;   // null = legacy array mode, array = column mode
    _data = null;       // raw row objects when in column mode
    _idField = null;
    _rowStylerFn = null;

    /**
     * Define columns for datagrid mode. Switches from array rows to object rows.
     * @param {Object[]} cols - [{field, title, width, hidden, align, formatter, style, cellClass, headerClass}]
     */
    columns(cols) {
      if (!arguments.length) return this._columns;
      this._columns = cols.map(c => ({ hidden: false, align: 'left', ...c }));
      this._rowTpl = null; // invalidate template
      this.setHeaders(this._visibleCols().map(c => c.title || c.field));
      return this;
    }
    ...
    _visibleCols() {
      return this._columns ? this._columns.filter(c => !c.hidden) : null;
    }

    _colByField(field) {
      return this._columns?.find(c => c.field === field) || null;
    }

    showColumn(field) {
      const col = this._colByField(field);
      if (col) { col.hidden = false; this._refreshFromColumns(); }
      return this;
    }

    hideColumn(field) {
      const col = this._colByField(field);
      if (col) { col.hidden = true; this._refreshFromColumns(); }
      return this;
    }
    ...
    _refreshFromColumns() {
      this._rowTpl = null; // invalidate template
      this.setHeaders(this._visibleCols().map(c => c.title || c.field));
      // Use _allData (full dataset) if available, else _data (current page)
      const src = this._allData || this._data;
      if (src) this.setData(src);
    }

    /**
     * Set or get the primary key field name (for selectRecord).
     */
    idField(field) {
    ...
    /**
     * Set a row styler function: (index, row) => {class, style} or null.
     */
    rowStyler(fn) {
      this._rowStylerFn = fn || null;
      return this;
    }

    /**
     * Select a row by its idField value.
     */
    selectRecord(id) {
      if (!this._idField || !this._data) return this;
      const idx = this._data.findIndex(r => r[this._idField] == id);
    ...
    setHeaders(arr) {
      ...
      if (vis) {
        for (const col of vis) {
          const parts = [];
          if (auto) { parts.push('width:1%', 'white-space:nowrap'); }
          else if (col.width) parts.push(typeof col.width === 'number' ? `width:${col.width}px` : `width:${col.width}`);
          if (col.align === 'right') parts.push('text-align:right');
          if (col.align === 'center') parts.push('text-align:center');
          if (col.headerStyle) parts.push(col.headerStyle);
          const style = parts.length ? ` style="${parts.join(';')}"` : '';
          const cls = col.headerClass ? ` class="${col.headerClass}"` : '';
          html += `<th${style}${cls}>${col.title || col.field}</th>`;
        }
      } else {
        const style = auto ? ' style="width:1%;white-space:nowrap"' : '';
        arr.forEach(h => { html += `<th${style}>${h}</th>`; });
      }
      if (auto) html += '<th></th>';
      html += '</tr>';
    ...
    getColCount() {
      if (this._columns) return this._visibleCols().length;
      return this.getHeaders().length || (this._rows()[0]?.children.length || 0);
    }
    getRowCount() { return this._rows().length; }

    getData() {
      // Column mode: return raw objects (includes hidden fields)
      if (this._data) return this._data.slice();

      // Legacy: read from DOM
      const auto = this._isAutoWidth();
    ...
    _isAutoWidth() {
      if (this.el.hasAttribute('data-autowidth')) return true;
      const th = this.el.querySelector('th');
      if (th?.style?.width === '1%' || th?.style?.whiteSpace === 'nowrap') return true;
      const ths = this.el.querySelectorAll('thead th');
      if (ths.length > 0 && ths[ths.length - 1]?.textContent.trim() === '') return true;
      return false;
    }

    /**
     * Build (or rebuild) the row <template> from current column config.
     * Called when columns change. In legacy mode, template is built per-render.
     */
    _buildRowTemplate() {
      const tpl = document.createElement('template');
      ...
      if (vis) {
        for (const col of vis) {
          ...
          // Static styles baked into template (dynamic styles applied per-row)
          const parts = [];
          if (auto) { parts.push('width:1%', 'white-space:nowrap'); }
          if (col.align === 'right') parts.push('text-align:right');
          if (col.align === 'center') parts.push('text-align:center');
          if (parts.length) td.setAttribute('style', parts.join(';'));
          tr.appendChild(td);
        }
      }
      if (auto) {
    ...
    /**
     * Create a <tr> element from a row of data using template cloning.
     */
    _createRow(row, rowIdx) {
      const vis = this._visibleCols();
      const auto = this._isAutoWidth();

      // Column mode: clone pre-built template
      if (vis) {
        if (!this._rowTpl) this._buildRowTemplate();
        const tr = this._rowTpl.content.firstElementChild.cloneNode(true);

        // Row styler
        if (this._rowStylerFn) {
          const rs = this._rowStylerFn(rowIdx, row);
          if (rs) {
            if (rs.class) tr.className = rs.class;
            if (rs.style) tr.style.cssText = (tr.style.cssText ? tr.style.cssText + ';' : '') + rs.style;
          }
        }

        // Populate cells — iterate visible columns, skip spacer td
        const tds = tr.querySelectorAll('td[data-field]');
        vis.forEach((col, i) => {
          const td = tds[i];
          const raw = row[col.field] ?? '';
          const val = col.formatter ? col.formatter(raw, row, rowIdx) : (raw === null || raw === undefined ? '' : raw);

          // Dynamic style/class (per-row)
          const colStyle = typeof col.style === 'function' ? col.style(raw, row, rowIdx) : col.style;
          if (colStyle) td.style.cssText = (td.style.cssText ? td.style.cssText + ';' : '') + colStyle;

          const colClass = typeof col.cellClass === 'function' ? col.cellClass(raw, row, rowIdx) : col.cellClass;
          if (colClass) td.className = colClass;

          // Set content — use innerHTML for formatters that return HTML
          if (col.formatter) td.innerHTML = val;
          else td.textContent = val;
        });

        // Edit icon in spacer column
        if (this._editorOpts) {
          const spacer = tr.querySelector('.dp-spacer');
          if (spacer) {
            spacer.innerHTML = '<svg class="dp-edit-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z"/></svg>';
            spacer.style.textAlign = 'right';
            spacer.querySelector('.dp-edit-icon').addEventListener('click', (e) => {
              e.stopPropagation();
              this.editRow(rowIdx);
            });
          }
      ...
      // Legacy array mode: build <tr> directly
      const tr = document.createElement('tr');
      if (this._rowStylerFn) {
        const rs = this._rowStylerFn(rowIdx, row);
        if (rs) {
          if (rs.class) tr.className = rs.class;
          if (rs.style) tr.style.cssText = rs.style;
        }
      }
      row.forEach(cell => {
        const td = document.createElement('td');
        if (auto) { td.style.width = '1%'; td.style.whiteSpace = 'nowrap'; }
        td.textContent = cell;
        tr.appendChild(td);
      });
      if (auto) tr.appendChild(document.createElement('td')); // spacer
      return tr;
    }

    setData(rows) {
      // Store raw data in column mode
      if (this._columns) this._data = rows;
      ...
      if (this._groupFn) {
        let lastGroup = null;
        rows.forEach((row, i) => {
          const groupKey = this._groupFn(row);
          if (groupKey !== lastGroup) {
            const label = this._groupFormatFn ? this._groupFormatFn(groupKey) : groupKey;
            const gtr = document.createElement('tr');
            gtr.className = 'dp-group-header';
            const gtd = document.createElement('td');
            gtd.setAttribute('colspan', '99');
            gtd.innerHTML = label;
            gtr.appendChild(gtd);
            tbody.appendChild(gtr);
            lastGroup = groupKey;
          }
          tbody.appendChild(this._createRow(row, i));
        });
      } else {
        rows.forEach((row, i) => tbody.appendChild(this._createRow(row, i)));
      }
    ...
    groupBy(fn, formatFn) {
      this._groupFn = fn || null;
      this._groupFormatFn = formatFn || null;
      return this;
    }

    /**
     * Group by column index (legacy) or field name (column mode).
     */
    groupByCol(colIndex, formatFn) {
      if (colIndex === null || colIndex === undefined) {
        this._groupFn = null;
        this._groupFormatFn = null;
      } else if (this._columns && typeof colIndex === 'string') {
        // Column mode: field name
        this._groupFn = (row) => row[colIndex] || '';
        this._groupFormatFn = formatFn || null;
      } else {
        // Legacy: numeric index
        this._groupFn = (row) => (Array.isArray(row) ? row[colIndex] : Object.values(row)[colIndex]) || '';
        this._groupFormatFn = formatFn || null;
      }
      return this;
    }
    ...
    getRow(n) {
      // Column mode: return object from _data
      if (this._data) return this._data[n] || null;
    ...
    /**
     * Get all loaded rows (alias for getData, matches DUI API).
     */
    getRows() { return this.getData(); }

    addRow(data, idx) {
      if (this._data) {
        // Column mode: insert into _data and re-render
        if (idx !== undefined) this._data.splice(idx, 0, data);
        else this._data.push(data);
        this.setData(this._data);
        return this;
      }
      // Legacy
      const tr = this._createRow(data, this.getRowCount());
      let tbody = this._tbody();
      if (!tbody) { tbody = document.createElement('tbody'); this.el.appendChild(tbody); }
    ...
    updateRow(n, data) {
      if (this._data && this._data[n]) {
        // Column mode: merge object
        if (typeof data === 'object' && !Array.isArray(data)) {
          Object.assign(this._data[n], data);
        } else {
          this._data[n] = data;
        }
        this.setData(this._data);
      ...
      // Legacy: replace the row element
      const rows = this._rows();
      if (rows[n]) {
        const newTr = this._createRow(data, n);
        rows[n].replaceWith(newTr);
      }
      this.emit('dp:datachange', this.getData());
      return this;
    }
    ...
    getCell(row, col) {
      // Column mode: col can be field name
      if (this._data && typeof col === 'string') {
        return this._data[row]?.[col] ?? null;
      }
    ...
    /**
     * Sort by column index (legacy) or field name (column mode).
     */
    sort(col, dir = 'asc') {
      const data = this.getData();
      data.sort((a, b) => {
        const va = typeof col === 'string' ? a[col] : a[col];
        const vb = typeof col === 'string' ? b[col] : b[col];
        const sa = String(va ?? ''), sb = String(vb ?? '');
        const cmp = isNaN(va) || isNaN(vb) ? sa.localeCompare(sb) : parseFloat(va) - parseFloat(vb);
    ...
    search(query) {
      ...
      if (this._columns) {
        return this.filter(row => Object.values(row).some(v => String(v).toLowerCase().includes(q)));
      }
      return this.filter(row => row.some(cell => String(cell).toLowerCase().includes(q)));
    }
    ...
    toCSV() {
      if (this._columns) {
        const vis = this._visibleCols();
        const headers = vis.map(c => c.title || c.field);
        const data = this.getData();
        const lines = [headers.join(',')];
        data.forEach(r => lines.push(vis.map(c => String(r[c.field] ?? '')).join(',')));
        return lines.join('\n');
      }
    ...
    toJSON() {
      // Column mode: data is already objects
      if (this._data) return this._data.slice();
    ...
    _initTableEvents() {
      ...
      (this._tbody() || this.el).addEventListener('click', (e) => {
        ...
        const tr = e.target.closest('tr:not(.dp-group-header)');
        if (!tr) return;
        ...
        if (td) {
          ...
          const value = field && this._data ? this._data[rowIdx]?.[field] : td.textContent.trim();
          this.emit('dp:cellclick', { value, field, rowIdx, colIdx });
        }
      ...
      (this._tbody() || this.el).addEventListener('dblclick', (e) => {
        ...
        const field = td?.dataset.field || null;
        const value = field && this._data ? this._data[rowIdx]?.[field] : td?.textContent.trim();
        this.emit('dp:rowdblclick', { data: this.getRow(rowIdx), index: rowIdx, field, value });
      });
    }
  }
  ...
  // ============================================================
  // DpPopup — externally triggered popup menu
  // ============================================================

  class DpPopup extends DpContainer {
    open(anchor) {
      this.el.style.display = '';
      // Position relative to anchor if provided
      if (anchor) {
        let anchorEl = typeof anchor === 'string' ? document.querySelector(anchor) : anchor;
        if (anchorEl instanceof DpComponent) anchorEl = anchorEl.el;
        if (anchorEl) {
          const rect = anchorEl.getBoundingClientRect();
          const pos = this.el.classList.contains('dp-popup-top') ? 'top' : 'bottom';
          if (pos === 'top') {
          ...
          // Horizontal alignment: anchor to right edge by default for top popups,
          // or when popup would overflow, or when dp-popup-end is set
          const forceEnd = this.el.classList.contains('dp-popup-end');
          const menuW = this.el.querySelector('ul, .menu')?.offsetWidth || 200;
          const wouldOverflow = rect.left + menuW > window.innerWidth;
          const isTopPopup = pos === 'top';
          if (forceEnd || wouldOverflow || isTopPopup) {
    ...
    toggle(anchor) {
      return this.isOpen() ? this.close() : this.open(anchor);
    }
    ...
    // Item methods (same as dropdown)
    _items() { return Array.from(this.el.querySelectorAll('.dp-dropdown-item, li')); }
    getItems() { return this._items().map(el => _wrap(el)); }
    getItemCount() { return this._items().length; }
    ...
    onItemClick(fn) {
      this.el.addEventListener('click', (e) => {
        ...
        const a = li.querySelector('a');
        this.close();
        fn({ text: li.textContent.trim(), page: a?.dataset.page, index: idx, href: a?.href }, e, this);
      });
      return this;
    }
  }
  ...
  class DpBadge extends DpComponent {
    getText() { return this.el.textContent.trim(); }
    setText(str) {
      const icon = this.el.querySelector('[data-lucide], svg');
      this.el.textContent = str;
      if (icon) this.el.prepend(icon);
      return this;
    }
    getColor() {
      for (const cls of this.el.classList) {
        const m = cls.match(/^badge-(\w+)$/);
        if (m && !['outline','dash','soft','ghost','xs','sm','md','lg','xl'].includes(m[1])) return m[1];
      }
      return null;
    }
    setColor(color) {
      const old = this.getColor();
      if (old) this.el.classList.remove(`badge-${old}`);
      if (color) this.el.classList.add(`badge-${color}`);
      return this;
    }
    getStyle() {
      for (const cls of this.el.classList) {
        if (/^badge-(outline|dash|soft|ghost)$/.test(cls)) return cls.slice(6);
      }
      return null;
    }
    setStyle(style) {
      const old = this.getStyle();
      if (old) this.el.classList.remove(`badge-${old}`);
      if (style) this.el.classList.add(`badge-${style}`);
      return this;
    }
    setIcon(name) {
      let icon = this.el.querySelector('[data-lucide]');
      if (icon) {
  ...
  class DpFormField extends DpComponent {
    getInput() {
      const input = this.el.querySelector('.dp-input, .dp-select, .dp-combobox, .dp-textarea, .dp-checkbox, .dp-toggle, .dp-radio, .dp-range, .dp-file-input');
  ...
  // ============================================================
  // DpFitem — DUI-style form field with pre/post slot API
  // ============================================================

  class DpFitem extends DpComponent {
    // --- Input access ---
    getInput() {
      const el = this.el.querySelector('.dp-input, .dp-select, .dp-combobox, .dp-textarea, .dp-checkbox, .dp-toggle, .dp-radio, .dp-range, .dp-file-input');
      return el ? _wrap(el) : null;
    }

    getValue() { return this.getInput()?.getValue(); }
    setValue(val) { this.getInput()?.setValue(val); return this; }
    clear() { this.getInput()?.clear(); return this; }
    focus() { this.getInput()?.focus(); return this; }

    // --- Label ---
    getLabel() {
      const lbl = this.el.querySelector('.dp-fitem-label');
      return lbl?.textContent.trim().replace(/\*$/, '').trim() || '';
    }

    setLabel(str) {
      const lbl = this.el.querySelector('.dp-fitem-label');
      if (lbl) {
        const req = lbl.querySelector('.text-error');
        lbl.textContent = str;
    ...
    enable() {
      ...
      if (input) input.el.disabled = false;
      const pre = this._pre();
      const post = this._post();
      if (pre) pre.style.opacity = '';
      if (post) { post.style.opacity = ''; post.style.pointerEvents = ''; }
      return this;
    }

    disable() {
      const input = this.getInput();
      if (input) input.el.disabled = true;
      const pre = this._pre();
      const post = this._post();
      if (pre) pre.style.opacity = '0.4';
      if (post) { post.style.opacity = '0.4'; post.style.pointerEvents = 'none'; }
      return this;
    }

    readOnly(bool = true) {
      const input = this.getInput();
      if (input) input.el.readOnly = bool;
      return this;
    }

    // --- Pre slot ---
    _pre() { return this.el.querySelector('.dp-fitem-pre'); }

    hasPre() { return !!this._pre(); }

    onPreClick(fn) {
      const pre = this._pre();
      if (pre) pre.addEventListener('click', (e) => fn(this.getValue(), e, this));
      return this;
    }
    ...
    setPreIcon(name) {
      const pre = this._pre();
      if (!pre) return this;
      const icon = pre.querySelector('[data-lucide]');
      if (icon) { icon.setAttribute('data-lucide', name); if (window.lucide) lucide.createIcons({ nodes: [icon] }); }
      return this;
    }

    showPre() { const p = this._pre(); if (p) p.style.display = ''; return this; }
    hidePre() { const p = this._pre(); if (p) p.style.display = 'none'; return this; }

    // --- Post slot ---
    _post() { return this.el.querySelector('.dp-fitem-post'); }

    hasPost() { return !!this._post(); }

    onPostClick(fn) {
      const post = this._post();
      if (post) post.addEventListener('click', (e) => fn(this.getValue(), e, this));
      return this;
    }
    ...
    setPostIcon(name) {
      const post = this._post();
      if (!post) return this;
      const icon = post.querySelector('[data-lucide]');
      if (icon) { icon.setAttribute('data-lucide', name); if (window.lucide) lucide.createIcons({ nodes: [icon] }); }
      return this;
    }

    showPost() { const p = this._post(); if (p) p.style.display = ''; return this; }
    hidePost() { const p = this._post(); if (p) p.style.display = 'none'; return this; }

    // --- Events ---
    onChange(fn) { this.getInput()?.onChange(fn); return this; }
    onInput(fn) { this.getInput()?.onInput(fn); return this; }
    onEnter(fn) { this.getInput()?.onEnter(fn); return this; }
    onEscape(fn) { this.getInput()?.onEscape(fn); return this; }

    // --- Properties ---
    getName() { return this.getInput()?.el?.name || ''; }
    isRequired() { return !!this.el.querySelector('.text-error'); }
    getWidgetType() {
      const input = this.getInput();
      if (!input) return null;
      for (const cls of input.el.classList) {
        if (cls.startsWith('dp-') && cls !== 'dp-input') return cls.slice(3);
      }
  ...
  class DpFormCard extends DpContainer {
    ...
    getData() {
      const data = {};
      this.el.querySelectorAll('.dp-input, .dp-select, .dp-combobox, .dp-textarea, .dp-checkbox, .dp-toggle').forEach(el => {
        const comp = _wrap(el);
    ...
    reset() {
      this.el.querySelectorAll('.dp-input, .dp-select, .dp-combobox, .dp-textarea').forEach(el => {
        _wrap(el).clear();
      });
      return this;
    }
  ...
  class DpModalForm extends DpModal {
    ...
    reset() {
      this.el.querySelectorAll('.dp-input, .dp-select, .dp-combobox, .dp-textarea').forEach(el => _wrap(el).clear());
      return this;
    }
    onSubmit(fn) {
  ...
  class DpDataTable extends DpTable {
    ...
    paginate(opts = {}) {
      ...
      // If data already loaded, re-render with pagination
    ...
    async loadPage(page) {
      ...
      if (this._columns) {
        // Column mode: keep rows as objects, auto-set headers on first page
        if (opts.headers !== false && page === 1 && !this.getHeaders().length) {
          this.setHeaders(this._visibleCols().map(c => c.title || c.field));
        }
      } else if (rows.length > 0 && !Array.isArray(rows[0])) {
        // Legacy: convert objects to arrays
      ...
      this._currentPage = page;
      // In server mode, render directly (not through our setData override)
      if (this._columns) this._data = rows;
      DpTable.prototype.setData.call(this, rows);
    ...
    _renderPage() {
      ...
      // Store page data for column-mode getRow/getData
      if (this._columns) this._data = page;
    ...
    _updatePaginationUI() {
      ...
      if (!pagDiv) {
        ...
        pagDiv.className = 'dp-datatable-pagination';
      ...
      prev.textContent = '\u2039';
      if (current <= 1) prev.className = 'disabled';
      else prev.addEventListener('click', () => this.setPage(current - 1));
      pagDiv.appendChild(prev);

      // Page info
      const info = document.createElement('span');
      info.className = 'dp-page-info';
      info.textContent = `${current} / ${pageCount}`;
      pagDiv.appendChild(info);

      // Next
      const next = document.createElement('button');
      next.textContent = '\u203a';
      if (current >= pageCount) next.className = 'disabled';
      else next.addEventListener('click', () => this.setPage(current + 1));
      pagDiv.appendChild(next);

      // Row count
      if (total > 0) {
        const count = document.createElement('span');
        count.className = 'dp-page-info';
        count.textContent = `${total} rows`;
        pagDiv.appendChild(count);
      }
    }

    // --- Override groupBy to re-render current page ---
    groupBy(fn, formatFn) {
      this._groupFn = fn || null;
      this._groupFormatFn = formatFn || null;
      if (this._allData) this._renderPage();
      return this;
    }
    ...
    // --- Override data methods to work on _allData ---

    getData() {
      // Return full dataset, not just current page
      if (this._columns && this._allData) return this._allData.slice();
      return this._allData ? this._allData.slice() : DpTable.prototype.getData.call(this);
    }

    sort(col, dir = 'asc') {
      if (!this._allData) return DpTable.prototype.sort.call(this, col, dir);
      this._allData.sort((a, b) => {
        const va = typeof col === 'string' ? a[col] : a[col];
        const vb = typeof col === 'string' ? b[col] : b[col];
        const sa = String(va ?? ''), sb = String(vb ?? '');
        const cmp = isNaN(va) || isNaN(vb) ? sa.localeCompare(sb) : parseFloat(va) - parseFloat(vb);
    ...
    filter(fn) {
      if (!this._origAllData) this._origAllData = this._allData ? this._allData.slice() : this.getData();
      this._allData = this._origAllData.filter(fn);
      this._totalRows = this._allData.length;
      this._currentPage = 1;
      this._renderPage();
      return this;
    }
    ...
    clearFilter() {
      if (this._origAllData) {
        this._allData = this._origAllData;
        this._origAllData = null;
        this._totalRows = this._allData.length;
        this._currentPage = 1;
        this._renderPage();
      }
    ...
    search(query) {
      ...
      return this.filter(row => (Array.isArray(row) ? row : Object.values(row)).some(v => String(v).toLowerCase().includes(q)));
    }

    selectRecord(id) {
      if (!this._idField || !this._allData) return this;
      const idx = this._allData.findIndex(r => r[this._idField] == id);
      if (idx < 0) return this;
      // Navigate to the page containing this row
      const page = Math.floor(idx / this._pageSize) + 1;
      if (page !== this._currentPage) {
        this._currentPage = page;
        this._renderPage();
      }
      // Select within current page
      const pageIdx = idx - (page - 1) * this._pageSize;
      this.selectRow(pageIdx);
      return this;
    }
    ...
    /**
     * Auto-build and wire an editor modal from column editor definitions.
     * @param {Object} opts - { addData, onEndEdit, onValidateRow, onDeleteRow }
     */
    rowEditor(opts = {}) {
      ...
      if (!this._editorDialog) this._buildEditorModal();

      // Re-render to inject edit icons into existing rows
      if (this._allData) this._renderPage();
    ...
    /**
     * Open editor in edit mode for row at index.
     */
    editRow(n) {
      const row = this._allData ? this._allData[n] : this.getRow(n);
      if (!row) return this;
      this._editorMode = 'edit';
      this._editorIndex = n;
      this._populateEditor(row);
      this._openEditorDialog();
      return this;
    }

    /**
     * Open editor in add mode with optional defaults.
     */
    addNewRow() {
      this._editorMode = 'add';
      this._editorIndex = -1;
      this._populateEditor(this._editorOpts?.addData || {});
      this._openEditorDialog();
      return this;
    }

    async _openEditorDialog() {
      // Show/hide delete button based on mode
      if (this._editorDelBtn) {
        this._editorDelBtn.style.display = this._editorMode === 'add' ? 'none' : '';
      }

      // Fire onBeforeRender callbacks (async support)
      const cols = this._columns || [];
      const row = this._editorMode === 'edit' && this._editorIndex >= 0
        ? (this._allData?.[this._editorIndex] || null) : null;
      const promises = [];
      for (const col of cols) {
        if (col.editor?.onBeforeRender && this._editorFields[col.field]) {
          const input = this._editorFields[col.field];
          const result = col.editor.onBeforeRender(input, row);
          if (result && typeof result.then === 'function') promises.push(result);
        }
      }
      if (promises.length) await Promise.all(promises);

      this._editorDialog.showModal();
      // Focus first editable input instead of close button
      const first = this._editorDialog.querySelector('input:not([readonly]):not([type=hidden]), .dp-cb-display, select, textarea');
      if (first) setTimeout(() => first.focus(), 0);
    }

    _buildEditorModal() {
      const cols = this._columns || [];
      const editorCols = cols.filter(c => c.editor);
      if (!editorCols.length) return;
      ...
      box.className = 'modal-box flex flex-col gap-0 p-0 max-w-lg';

      // Titlebar
      const titlebar = document.createElement('div');
      titlebar.className = 'flex items-center gap-2 px-4 h-10 bg-primary text-primary-content text-sm font-semibold';
      titlebar.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z"/></svg>';
      const titleSpan = document.createElement('span');
      titleSpan.className = 'flex-1';
      titleSpan.textContent = 'Edit Row';
      titlebar.appendChild(titleSpan);
      const closeX = document.createElement('button');
      closeX.className = 'btn btn-ghost btn-xs btn-circle text-primary-content';
      closeX.textContent = '\u2715';
      ...
      for (const col of cols) {
        const hasEditor = !!col.editor;
        if (col.hidden && !hasEditor) {
          // Hidden data field — round-trip as hidden input
          const hidden = document.createElement('input');
          hidden.type = 'hidden';
          hidden.name = col.field;
          form.appendChild(hidden);
          this._editorFields[col.field] = hidden;
          continue;
        }
        ...
        fitem.className = 'flex flex-col gap-1';
        ...
        if (type === 'textarea') {
          ...
          if (ed.rows) input.rows = ed.rows;
        } else if (type === 'combobox') {
          input = document.createElement('div');
          input.className = 'dp-combobox w-full';
          // Sub-elements built by _ensureStructure at wrap time
          const cbDisplay = document.createElement('input');
          cbDisplay.type = 'text';
          cbDisplay.readOnly = true;
          cbDisplay.classList.add('dp-cb-display', 'input', 'input-bordered', 'input-sm', 'w-full');
          const cbHidden = document.createElement('input');
          cbHidden.type = 'hidden';
          const cbToggle = document.createElement('button');
          cbToggle.type = 'button';
          cbToggle.classList.add('dp-cb-toggle');
          cbToggle.tabIndex = -1;
          cbToggle.innerHTML = _CB_CHEVRON;
          const cbDropdown = document.createElement('ul');
          cbDropdown.classList.add('dp-cb-dropdown', 'menu');
          input.append(cbDisplay, cbHidden, cbToggle, cbDropdown);
          if (ed.data) {
            input._pendingData = ed.data;
          }
          if (ed.valueField || ed.textField) {
            input._pendingConfig = { valueField: ed.valueField, textField: ed.textField };
          }
        } else if (type === 'datebox') {
          input = document.createElement('input');
          input.type = 'date';
          input.className = 'input input-bordered input-sm w-full';
        } else if (type === 'numberbox' || type === 'numberspinner') {
          input = document.createElement('input');
          input.type = 'number';
          input.className = 'input input-bordered input-sm w-full';
          if (ed.min !== undefined) input.min = ed.min;
          if (ed.max !== undefined) input.max = ed.max;
          if (ed.precision !== undefined) input.step = ed.precision === 0 ? '1' : (1 / Math.pow(10, ed.precision)).toString();
        } else {
          // textbox default
          input = document.createElement('input');
          input.type = 'text';
        ...
        // For combobox, name goes on hidden input; others get name directly
        if (type === 'combobox') {
          input.querySelector('input[type="hidden"]').name = col.field;
          if (ed.readonly) input.querySelector('.dp-cb-display').readOnly = true;
          if (ed.required) input.querySelector('input[type="hidden"]').required = true;
        } else {
          input.name = col.field;
          if (ed.readonly) input.readOnly = true;
          if (ed.required) input.required = true;
        }

        fitem.appendChild(input);
        form.appendChild(fitem);
        this._editorFields[col.field] = input;
      }
      ...
      footer.className = 'flex gap-2 px-4 py-2 border-t border-base-200';
      ...
      // Delete button — only if onDeleteRow provided, hidden in add mode
      if (this._editorOpts.onDeleteRow) {
        const delBtn = document.createElement('button');
        delBtn.type = 'button';
        delBtn.className = 'btn btn-error btn-outline btn-sm';
        delBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg> Delete';
        delBtn.addEventListener('click', () => {
          if (!confirm('Delete this row?')) return;
          const idx = this._editorIndex;
          const row = this._allData?.[idx] || this.getRow(idx);
          dialog.close();
          if (this._allData && idx >= 0) {
            this._allData.splice(idx, 1);
            this._totalRows = this._allData.length;
            this._renderPage();
          }
          this._editorOpts.onDeleteRow({ index: idx, row });
          this.emit('dp:datachange', this.getData());
        });
        this._editorDelBtn = delBtn;
      ...
      // Auto-discover combobox elements and load pending static data
      dialog.querySelectorAll('.dp-combobox').forEach(el => {
        const combo = _wrap(el);
        if (el._pendingConfig) combo.config(el._pendingConfig);
        if (el._pendingData) combo.loadData(el._pendingData);
      });
    }

    _populateEditor(row) {
      if (!this._editorFields) return;
      for (const [field, el] of Object.entries(this._editorFields)) {
        const val = row[field] ?? '';
        if (el.classList?.contains('dp-combobox')) {
          _wrap(el).setValue(val);
        } else {
          el.value = val;
        }
      }
    ...
    _saveEditor() {
      const opts = this._editorOpts || {};
      const formData = this._collectEditor();
      const mode = this._editorMode;
      const index = this._editorIndex;

      // Merge: existing row + form data
      let row;
      if (mode === 'edit' && index >= 0) {
        const existing = this._allData ? this._allData[index] : {};
        row = { ...existing, ...formData };
      } else {
        row = { ...(opts.addData || {}), ...formData };
      }

      // Validate
      if (typeof opts.onValidateRow === 'function') {
        const err = opts.onValidateRow({ index, row, formData, mode });
        if (err) {
          // Show error — find or create error display
          alert(err); // TODO: replace with dp.toast or messager
          return;
        }
      }
      ...
      if (mode === 'add') {
        if (this._allData) {
          this._allData.push(row);
          this.setData(this._allData);
        } else {
          this.addRow(row);
        }
      } else if (mode === 'edit' && index >= 0) {
        if (this._allData && this._allData[index]) {
          Object.assign(this._allData[index], formData);
          this._renderPage();
        }
      }
      ...
      if (typeof opts.onEndEdit === 'function') {
        opts.onEndEdit({ mode, index: mode === 'add' ? this.getTotalRows() - 1 : index, row, formData });
      }

      this.emit('dp:datachange', this.getData());
    }

    onEndEdit(fn) { return this.on('dp:endedit', fn); }
    onDeleteRow(fn) { return this.on('dp:deleterow', fn); }
  }
  ...
  const REGISTRY = {
    ...
    'input': DpInput, 'select': DpSelect, 'combobox': DpCombobox, 'textarea': DpTextarea,
    ...
    'tooltip': DpTooltip, 'navbar': DpNavbar, 'dock': DpDock, 'popup': DpPopup,
    ...
    // Composites
    'fitem': DpFitem, 'formfield': DpFormField, 'formcard': DpFormCard,
  ...
  function _wrap(el) {
    ...
    // Post-init hooks — bind table events, but skip inner tables owned by DpDataTable
    if (comp instanceof DpTable && !el.closest('.dp-datatable')) {
      comp._initTableEvents();
    }
    if (comp instanceof DpDataTable) {
      comp._initTableEvents();
    }
    if (comp instanceof DpCombobox) {
      comp._ensureStructure();
      comp._initComboEvents();
    }
  ...
  class DpForm {
    constructor(container) {
      ...
      // Collect form-parts (forms with data-form-part matching this form's id)
      this._parts = [];
      const formId = this.el.id;
      if (formId) {
        this._parts = Array.from(document.querySelectorAll(`[data-form-part="${formId}"]`));
      }
    ...
    // Get all containers (main + parts)
    _containers() {
      return [this.el, ...this._parts];
    }

    // Find input by name, id, or placeholder — searches main + parts
    _find(name, type) {
      const selectors = [
        `[name="${name}"]`,
        `#${name}`,
    ...
    _findByType(name, dpClass) {
      ...
      // Fallback: search by dp class + name across all containers
      for (const container of this._containers()) {
        const el = container.querySelector(`.dp-${dpClass}[name="${name}"]`) ||
                   container.querySelector(`.dp-${dpClass}#${name}`);
        if (el) return _wrap(el);
      }
      return null;
    }
    ...
    // Get all field components across main + parts
    fields() {
      const inputs = [];
      const sel = '.dp-input, .dp-select, .dp-combobox, .dp-textarea, .dp-checkbox, .dp-toggle, .dp-radio, .dp-range, .dp-file-input';
      for (const container of this._containers()) {
        inputs.push(...Array.from(container.querySelectorAll(sel)));
      }
      return inputs.map(el => _wrap(el));
    }
    ...
    // ================================================================
    // Form State Machine: view / add / edit
    // ================================================================

    /**
     * Bind this form to a toolbar for auto-wiring CRUD buttons.
     * @param {string|Element} toolbar - selector or element of the toolbar
     * @param {object} opts - { url: '/api/endpoint' }
     */
    bind(toolbar, opts = {}) {
      this._toolbar = typeof toolbar === 'string' ? document.querySelector(toolbar) : toolbar;
      this._url = opts.url || null;
      this._idField = opts.idField || null;

      // Wire toolbar buttons
      const tbId = this._toolbar?.id?.replace('_toolbar', '') || '';
      const btn = (suffix) => this._toolbar?.querySelector(`#${tbId}_${suffix}`) || document.getElementById(`tb_${suffix}`);
      ...
      // Add button → mode('add')
      if (this._btns.add) this._btns.add.addEventListener('click', () => this.mode('add'));
      // Save button → save
      if (this._btns.save) this._btns.save.addEventListener('click', () => this._doSave());
      // Cancel button → mode('view')
      if (this._btns.cancel) this._btns.cancel.addEventListener('click', () => this._doCancel());
      // Edit button → mode('edit')
      if (this._btns.edit) this._btns.edit.addEventListener('click', () => this.mode('edit'));
      // Delete button → delete
      if (this._btns.del) this._btns.del.addEventListener('click', () => this._doDelete());
    ...
    /**
     * Get or set form mode: 'view', 'add', 'edit'
     */
    mode(newMode) {
      ...
      // Snapshot on entering edit (for cancel)
      if (newMode === 'edit' && prev === 'view') {
        this.snapshot();
      }
      ...
      if (this.el.dispatchEvent) {
        this.el.dispatchEvent(new CustomEvent('dp:modechange', { detail: { mode: newMode, prev }, bubbles: true }));
      }

      if (this._onModeChange) this._onModeChange(newMode, prev, this);
      return this;
    }

    onModeChange(fn) { this._onModeChange = fn; return this; }

    _updateButtons() {
      const m = this._mode;
      const en = (btn) => { if (btn) { btn.disabled = false; btn.classList.remove('btn-disabled', 'opacity-40', 'pointer-events-none'); } };
      const dis = (btn) => { if (btn) { btn.disabled = true; btn.classList.add('btn-disabled'); } };

      if (m === 'view') {
        en(this._btns.add);
        dis(this._btns.save);
        dis(this._btns.cancel);
        en(this._btns.edit);
        dis(this._btns.del);
        en(this._btns.print);
      } else if (m === 'add') {
        dis(this._btns.add);
        en(this._btns.save);
        en(this._btns.cancel);
        dis(this._btns.edit);
        dis(this._btns.del);
        dis(this._btns.print);
      } else if (m === 'edit') {
        dis(this._btns.add);
        en(this._btns.save);
        en(this._btns.cancel);
    ...
    async _doSave() {
      ...
      if (this._url) {
        const method = this._mode === 'add' ? 'POST' : 'PUT';
        const url = this._mode === 'edit' && this._idField
          ? `${this._url}/${this.get.value(this._idField)}`
          : this._url;
        await _fetch(method, url, this.getData());
      }

      this.snapshot();
      this.mode('view');

      if (this._onSave) this._onSave(this.getData(), this);
    }
    ...
    async _doDelete() {
      ...
      if (this._url && this._idField) {
        const id = this.get.value(this._idField);
        if (id) await _fetch('DELETE', `${this._url}/${id}`);
      }
    ...
    onDelete(fn) { this._onDelete = fn; return this; }

    // Warn before leaving page with unsaved changes
    guardNavigation() {
      window.addEventListener('beforeunload', (e) => {
        if (this._mode !== 'view' && this.isDirty()) {
          e.preventDefault();
          e.returnValue = '';
        }
      });
  ...
  DpComponent.prototype._gatherData = function() {
    ...
    const data = {};
    this.el.querySelectorAll('.dp-input, .dp-select, .dp-combobox, .dp-textarea, .dp-checkbox, .dp-toggle, .dp-radio, .dp-range').forEach(el => {
      const comp = _wrap(el);
  ...
  // ============================================================
  // Remember — persistent component state via localStorage
  // ============================================================

  const _remPrefix = 'dp:rem:';
  const _remTTL = 30; // days

  function _remScope() {
    return window.location.pathname.replace(/\//g, '_') || '_root';
  }

  function _remKey(id, scope) {
    return `${_remPrefix}${scope || _remScope()}:${id}`;
  }

  dp.remember = function(selector) {
    const el = typeof selector === 'string' ? document.querySelector(selector) : selector;
    if (!el) return;
    const id = el.id;
    if (!id) return;

    // Add remember class if not present
    el.classList.add('remember');

    // Restore saved state
    const saved = dp.remember.get(id);
    if (saved !== null) {
      const comp = _wrap(el);
      _remRestore(comp, saved);
    }
  ...
  // Get saved value
  dp.remember.get = function(id, scope) {
    const key = _remKey(id, scope);
    try {
      const raw = localStorage.getItem(key);
      if (!raw) return null;
      const entry = JSON.parse(raw);
      // Check TTL
      if (_remTTL > 0 && entry.t) {
        const age = (Date.now() - entry.t) / (1000 * 60 * 60 * 24);
        if (age > _remTTL) {
  ...
  // Save value
  dp.remember.set = function(id, value, scope) {
    const key = _remKey(id, scope);
    try {
  ...
  // Remove saved value
  dp.remember.remove = function(id, scope) {
    localStorage.removeItem(_remKey(id, scope));
  };

  // Clear all remembered state for a scope (or current page)
  dp.remember.clear = function(scope) {
    const prefix = `${_remPrefix}${scope || _remScope()}:`;
    Object.keys(localStorage).forEach(k => {
      if (k.startsWith(prefix)) localStorage.removeItem(k);
    });
  };

  // Clear all remembered state everywhere
  dp.remember.clearAll = function() {
    Object.keys(localStorage).forEach(k => {
      if (k.startsWith(_remPrefix)) localStorage.removeItem(k);
    });
  };

  // TTL in days (settable)
  dp.remember.ttl = _remTTL;

  // Restore component state from saved value
  function _remRestore(comp, value) {
    if (comp instanceof DpTabs) {
      comp.setActive(value);
    } else if (comp instanceof DpCheckbox || comp instanceof DpToggle) {
      comp.setChecked(value);
    } else if (comp instanceof DpSelect) {
      comp.setValue(value);
    } else if (comp instanceof DpRange) {
      comp.setValue(value);
    } else if (comp instanceof DpInput) {
      comp.setValue(value);
    } else if (comp instanceof DpDock) {
      comp.setActive(value);
    } else if (comp instanceof DpVsplit || comp instanceof DpHsplit) {
      // Restore panel size
      const prop = comp instanceof DpVsplit ? 'gridTemplateColumns' : 'gridTemplateRows';
      comp.el.style[prop] = value;
    } else if (comp instanceof DpMenu) {
      comp.setActive(value);
    } else if (comp instanceof DpRating) {
      comp.setValue(value);
    }
  }

  // Watch component for changes and auto-save
  function _remWatch(comp) {
    const id = comp.el.id;
    if (!id) return;

    if (comp instanceof DpTabs) {
      // Watch tab radio changes
      comp.el.addEventListener('change', () => {
        const active = comp.getActive();
        dp.remember.set(id, active);
      });
    } else if (comp instanceof DpCheckbox || comp instanceof DpToggle) {
      comp.el.addEventListener('change', () => dp.remember.set(id, comp.isChecked()));
    } else if (comp instanceof DpSelect) {
      comp.el.addEventListener('change', () => dp.remember.set(id, comp.getValue()));
    } else if (comp instanceof DpRange) {
      comp.el.addEventListener('input', () => dp.remember.set(id, comp.getValue()));
    } else if (comp instanceof DpInput) {
      comp.el.addEventListener('change', () => dp.remember.set(id, comp.getValue()));
    } else if (comp instanceof DpDock) {
      comp.el.addEventListener('click', () => {
        setTimeout(() => dp.remember.set(id, comp.getActive()), 0);
      });
    } else if (comp instanceof DpMenu) {
      comp.el.addEventListener('click', () => {
        setTimeout(() => dp.remember.set(id, comp.getActiveIndex()), 0);
      });
    } else if (comp instanceof DpRating) {
      comp.el.addEventListener('change', () => dp.remember.set(id, comp.getValue()));
    }
  ...
  // Auto-init: find all .remember elements after discovery
  function _remAutoInit() {
    document.querySelectorAll('.remember[id]').forEach(el => {
      dp.remember(el);
    });
  }

  // Convenience: opt-in all .remember elements
  dp.rememberAll = _remAutoInit;
  ...
  function _discover() {
    ...
    _remAutoInit(); // Auto-init remember after component discovery
    _isReady = true;
