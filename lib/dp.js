...
(function(global) {
  ...
  // ============================================================
  // DpCombobox — select with data store, remote loading, DUI-compatible API
  // ============================================================

  class DpCombobox extends DpSelect {
    _cbData = null;    // loaded data rows [{value, text, ...}]
    _cbOpts = { valueField: 'value', textField: 'text' };

    /**
     * Configure combobox options.
     * @param {Object} opts - {valueField, textField, url, queryParams, loadFilter}
     */
    config(opts) {
    ...
    /**
     * Load data from array. Renders <option> elements via DocumentFragment.
     * @param {Array|Object} data - [{value, text}] or {rows: [...]}
     */
    loadData(data) {
      // Unwrap {rows: [...]}
      ...
      // Normalize rows — fallback field names
      ...
      // Preserve placeholder (first disabled+selected option)
      const placeholder = this.el.querySelector('option[disabled][selected]');

      // Build options via DocumentFragment
      const frag = document.createDocumentFragment();
      if (placeholder) frag.appendChild(placeholder.cloneNode(true));

      const tpl = document.createElement('template');
      tpl.content.appendChild(document.createElement('option'));

      for (const row of this._cbData) {
        const opt = tpl.content.firstElementChild.cloneNode(true);
        opt.value = row[vf];
        opt.textContent = row[tf];
    ...
    /**
     * Fetch data from URL, then loadData.
     * @param {string} url
     * @param {Object} opts - {valueField, textField, queryParams, method, loadFilter}
     */
    async load(url, opts = {}) {
    ...
    /**
     * Re-fetch from configured URL (or new URL).
     */
    async reload(url) {
    ...
    /**
     * Get full data row for a value. No arg = current selection.
     */
    getRec(value) {
    ...
    /**
     * Check if value exists in loaded data.
     */
    exists(value) {
    ...
    /**
     * Override setValue — queue autoload if value not in current options.
     */
    setValue(val) {
      this.el.value = val;
      if (this.el.value !== String(val) && this._cbData) {
        // Value not in options — queue for after next loadData
        this._autoload = val;
      }
      return this;
    }
    ...
    /**
     * Get selected option text (DUI-compatible alias).
     */
     ...
     */
    clear() {
      this.el.selectedIndex = 0; // back to placeholder if exists
      return this;
    }
    ...
    /**
     * Init: bridge native change → dp:select with full row data.
     */
    _initComboEvents() {
      this.el.addEventListener('change', () => {
        const val = this.getValue();
        const rec = this.getRec(val);
        const text = this.getSelectedText();
        this.emit('dp:select', { value: val, text, rec, index: this.el.selectedIndex });
      });
    }
  }
  ...
  class DpFitem extends DpComponent {
    // --- Input access ---
    getInput() {
      const el = this.el.querySelector('.dp-input, .dp-select, .dp-textarea, .dp-checkbox, .dp-toggle, .dp-radio, .dp-range, .dp-file-input');
      return el ? _wrap(el) : null;
    }
  ...
  class DpDataTable extends DpTable {
    ...
    async _openEditorDialog() {
      ...
      const first = this._editorDialog.querySelector('input:not([readonly]):not([type=hidden]), select, textarea');
    ...
    _buildEditorModal() {
      ...
      for (const col of cols) {
        ...
        if (type === 'textarea') {
          ...
          input.className = 'dp-combobox select select-bordered select-sm w-full';
          // Static data loaded after modal built via DpCombobox.loadData
          if (ed.data) {
    ...
    _populateEditor(row) {
      ...
      for (const [field, input] of Object.entries(this._editorFields)) {
        const val = row[field] ?? '';
        input.value = val;
      }
    }
  ...
  class DpForm {
    ...
    fields() {
      const inputs = [];
      const sel = '.dp-input, .dp-select, .dp-textarea, .dp-checkbox, .dp-toggle, .dp-radio, .dp-range, .dp-file-input';
      for (const container of this._containers()) {
