  ...
  class DpTable extends DpComponent {
    ...
    _rowHtml(row, rowIdx) {
      ...
      if (this._rowStylerFn) {
        ...
        if (rs) {
          if (rs.class) html += ` class="${rs.class}"`;
          if (rs.style) html += ` style="${rs.style}"`;
        }
      }
      html += '>';

      if (vis) {
        // Column mode — render visible columns with formatting
        for (const col of vis) {
          ...
          if (colStyle) parts.push(colStyle);
          const style = parts.length ? ` style="${parts.join(';')}"` : '';

          const colClass = typeof col.cellClass === 'function' ? col.cellClass(raw, row, rowIdx) : col.cellClass;
          const cls = colClass ? ` class="${colClass}"` : '';

          html += `<td data-field="${col.field}"${style}${cls}>${val}</td>`;
        }
      } else {
        // Legacy array mode
        html += row.map(cell => {
      ...
      if (this._groupFn) {
        ...
        rows.forEach((row, i) => {
          ...
          if (groupKey !== lastGroup) {
            ...
            html += `<tr class="dp-group-header"><td colspan="99">${label}</td></tr>`;
            lastGroup = groupKey;
          }
          html += this._rowHtml(row, i);
        });
        tbody.innerHTML = html;
      } else {
        tbody.innerHTML = rows.map((row, i) => this._rowHtml(row, i)).join('');
      }
