...
(function(global) {
  ...
  class DpTable extends DpComponent {
    ...
    setHeaders(arr) {
      ...
      thead.innerHTML = '<tr>' + arr.map(h => `<th${style}>${h}</th>`).join('') + (auto ? '<th></th>' : '') + '</tr>';
      return this;
    }
    ...
    _isAutoWidth() {
      // data-autowidth attribute is the definitive source (set by table mixin)
      if (this.el.hasAttribute('data-autowidth')) return true;
      // Fallback: check th styles or spacer
    ...
    setData(rows) {
      ...
      if (this._groupFn) {
        ...
        const colCount = this.getColCount() || (rows[0]?.length || 1);
        const auto = this._isAutoWidth();
        const totalCols = auto ? colCount + 1 : colCount;
        let lastGroup = null;
        let html = '';
        ...
        for (const row of rows) {
          ...
          if (groupKey !== lastGroup) {
            ...
            html += `<tr class="dp-group-header"><td colspan="${totalCols}" style="font-weight:600;font-size:0.75rem;padding:0.5rem 0.75rem;opacity:0.6;border-bottom:1px solid color-mix(in oklch,currentColor 15%,transparent)">${label}</td></tr>`;
            lastGroup = groupKey;
          }
          html += this._rowHtml(row);
        }
    ...
    /**
     * Group rows by a function. Inserts header rows when group key changes.
     * @param {function|null} fn - receives row array, returns group key string. null to clear.
     * @param {function} formatFn - optional: receives key, returns display label
     */
    groupBy(fn, formatFn) {
     ...
     * @param {number} colIndex - column index to group by
     * @param {function} formatFn - optional: receives value, returns display label
     */
