...
(function(global) {
  ...
  class DpTable extends DpComponent {
    ...
    addRow(data, idx) {
      ...
      tr.innerHTML = data.map(cell => `<td>${cell}</td>`).join('');
      let tbody = this._tbody();
      if (!tbody) { tbody = document.createElement('tbody'); this.el.appendChild(tbody); }
  ...
  class DpDataTable extends DpTable {
    // Pagination on top of DpTable
    _pageSize = 10;
    _currentPage = 1;

    getPage() { return this._currentPage; }
    setPage(n) { this._currentPage = Math.max(1, Math.min(n, this.getPageCount())); this._renderPage(); return this; }
    getPageCount() { return Math.ceil((this._allData || this.getData()).length / this._pageSize); }
    getPageSize() { return this._pageSize; }
    setPageSize(n) { this._pageSize = n; this._currentPage = 1; this._renderPage(); return this; }
    ...
    _renderPage() {
      ...
      this.setData(page);
      this.emit('dp:pagechange', this._currentPage);
    }
  }
  ...
  const REGISTRY = {
    ...
    'tooltip': DpTooltip, 'navbar': DpNavbar,
  ...
  class DpForm {
    ...
    validate() {
      ...
      this.fields().forEach(f => {
        const name = f.el.name || f.el.id || '';
        if (!name) return;
        if (f.el.required && !f.getValue()) {
          errors[name] = 'Required';
          valid = false;
        }
        if (f.el.type === 'email' && f.getValue() && !f.getValue().includes('@')) {
          errors[name] = 'Invalid email';
          valid = false;
        }
        if (f.el.minLength && f.getValue().length < f.el.minLength) {
